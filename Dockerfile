...
RUN mkdir -p /sources/git && cd /sources/git && git clone https://github.com/mardambey/pinot.git && cd pinot && git checkout contrib-confluent-platform-2.0.0 && /opt/maven/apache-maven-3.3.9/bin/mvn install package -DskipTests && cd pinot-distribution/target/pinot-0.016-pkg && chmod +x bin/*.sh && cd .. && mv pinot-0.016-pkg /opt && cd / && rm -Rf /sources && rm -Rf /root/.m2
